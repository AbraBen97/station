...
def load_data_from_excel():
    ...
    try:
        ...
        if os.path.exists('prices.xlsx'):
            ...
            st.session_state.prices = prices_df.set_index('fuel_type')['price'].to_dict()
...
def save_data_to_excel():
    ...
    try:
        ...
        prices_data = [{'fuel_type': k, 'price': v} for k, v in st.session_state.prices.items()]
...
def admin_dashboard():
    ...
    with tabs[3]:  # Livraisons
        ...
        with col1:
            ...
            if st.button("Ajouter livraison"):
                ...
                # Ajouter aux enregistrements (vous pouvez créer une liste séparée pour les livraisons)
    ...
    with tabs[6]:  # Paramètres
        ...
        with col1:
            ...
            new_gazoil_price = st.number_input("Prix Gazoil (CFA/L)", value=st.session_state.prices['gazoil'], step=0.01)
            new_super_price = st.number_input("Prix Super (CFA/L)", value=st.session_state.prices['super'], step=0.01)
            ...
            if st.button("➕ Ajouter une station"):
                if new_station_name and new_station_name not in st.session_state.stations:
                    ...
                    st.session_state.inventory[new_station_name] = {'gazoil': 0, 'super': 0}
                    save_data_to_excel()
        ...
        with col2:
            ...
            if st.button("📊 Exporter toutes les données"):
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    ...
                    # Prix
                    prices_data = [{'Carburant': k, 'Prix': v} for k, v in st.session_state.prices.items()]
                    prices_df = pd.DataFrame(prices_data)
